...
const RecipeSuggestions: React.FC<RecipeSuggestionsProps> = ({ 
  expiringItems, allItems, favorites, onToggleFavorite, lang 
}) => {
  ...
  const fetchRecipes = useCallback(async (force: boolean = false) => {
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-700 pb-10">
      ...
      {adPlaying && (
        <div className="fixed inset-0 bg-slate-900 z-[100] flex flex-col items-center justify-center p-8 animate-in fade-in duration-300">
          ...
          <p className="text-white/40 text-xs font-bold uppercase tracking-widest">{t.adLoading}</p>
        </div>
      )}
      ...
      <div className="flex bg-white p-1 rounded-2xl border-2 border-slate-100 mb-4">
        <button 
          ...
          className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeSubTab === 'ai' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-400'}`}
        >
          ✨ AI
        </button>
        <button 
          onClick={() => setActiveSubTab('fav')}
          className={`flex-1 py-3 rounded-xl font-black text-sm transition-all ${activeSubTab === 'fav' ? 'bg-rose-500 text-white shadow-md' : 'text-slate-400'}`}
        >
          ❤️ {t.favorites}
        </button>
      </div>
      ...
      {!loading && !adPlaying && displayList.length === 0 && (
        <div className="text-center py-20 opacity-40">
          ...
          <p className="font-bold">{activeSubTab === 'ai' ? t.noItems : t.noFavs}</p>
        </div>
      )}

      {!adPlaying && displayList.map((recipe, idx) => (
        <div key={recipe.id || idx} className="bg-white rounded-[35px] shadow-sm border-2 border-slate-100 overflow-hidden mb-6">
          <div className="p-6 bg-slate-50 flex justify-between items-start">
            <div className="flex-1 pr-4">
              ...
              <p className="text-emerald-700 text-xs font-bold mt-1">✓ {recipe.reason}</p>
            </div>
            <button 
              onClick={() => onToggleFavorite(recipe)}
              className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all border-2 ${favorites.find(f => f.id === recipe.id) ? 'bg-rose-50 border-rose-100 text-rose-500' : 'bg-white border-slate-100 text-slate-300'}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill={favorites.find(f => f.id === recipe.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>
            </button>
          </div>
          <div className="p-6 space-y-4">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.ingredients}</p>
              <div className="flex flex-wrap gap-2">
                {recipe.ingredients.map((ing, i) => (
                  <span key={i} className="bg-slate-50 text-slate-600 px-3 py-1.5 rounded-xl text-xs font-bold border border-slate-100">{ing}</span>
                ))}
              </div>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.steps}</p>
              <div className="space-y-3">
                {recipe.instructions.map((step, i) => (
                  <div key={i} className="flex gap-3 items-start">
                    <span className="w-6 h-6 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center text-[10px] font-black flex-shrink-0">{i + 1}</span>
      ...
      {activeSubTab === 'ai' && !loading && !adPlaying && recipes.length > 0 && (
        <div className="space-y-3">
          ...
          >
            {localStorage.getItem(DATE_KEY) === new Date().toDateString() ? `📺 ${t.watchAd}` : `🔄 ${t.refreshAI}`}
          </button>
        </div>
      )}
    </div>
