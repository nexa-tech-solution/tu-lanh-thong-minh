...
const Inventory: React.FC<InventoryProps> = ({ items, removeItem, lang }) => {
  ...
  const categories: (Category | 'Tất cả')[] = ['Tất cả', 'Thịt & Hải sản', 'Rau củ', 'Trái cây', 'Sữa & Trứng', 'Gia vị', 'Khác'];
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-6 duration-700">
      <div className="space-y-4">
        ...
        <div className="relative group">
          ...
          <div className="absolute left-5 top-1/2 -translate-y-1/2 text-orange-400 group-focus-within:scale-110 transition-transform">
            <svg xmlns="http://www.w3.org/2000/center" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
          </div>
          {searchTerm && (
            <button onClick={() => setSearchTerm('')} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
          {categories.map(cat => (
            <button
              ...
              className={`px-6 py-2.5 rounded-2xl text-[11px] font-black tracking-widest uppercase whitespace-nowrap transition-all border-2 ${selectedCategory === cat ? 'bg-orange-600 border-orange-600 text-white shadow-lg shadow-orange-100' : 'bg-white border-slate-100 text-slate-400 hover:border-orange-200'}`}
            >
              {cat === 'Tất cả' ? (t.all || 'Tất cả') : (t.categories[cat as Category] || cat)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4 pt-2">
        {filteredItems.map(item => {
          ...
          return (
            <div key={item.id} className="bg-white p-5 rounded-[30px] shadow-sm border-2 border-slate-50 flex items-center justify-between active:bg-orange-50/50 transition-all hover:border-orange-100">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs font-bold text-slate-400">{item.quantity} {item.unit}</span>
                    <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                    <span className={`text-[10px] font-black uppercase tracking-tight ${isExpired ? 'text-rose-500' : 'text-slate-300'}`}>
                      {new Date(item.expiryDate).toLocaleDateString(lang === 'vi' ? 'vi-VN' : (lang === 'en' ? 'en-US' : 'ja-JP'))}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
            ...
            <p className="text-sm font-bold text-slate-300 uppercase tracking-widest">{t.noItems || 'Không tìm thấy gì'}</p>
          </div>
        )}
      </div>
    </div>
