...
const ScannerModal: React.FC<ScannerModalProps> = ({ lang, onClose, onScan }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/95 z-[80] flex flex-col items-center justify-center p-8 animate-in fade-in duration-300">
      <div className="absolute top-8 right-8">
        <button onClick={onClose} className="w-14 h-14 bg-white/10 text-white rounded-full flex items-center justify-center hover:bg-white/20 transition-all">
      ...
      <div className="mt-12 w-full max-w-sm">
        ...
        >
          {scanning ? (lang === 'vi' ? 'Quét Thử Ngay' : 'Try Scanning') : (lang === 'vi' ? 'Đang xử lý...' : 'Processing...')}
        </button>
      </div>
